...
def get_sampling_fn(config, noise_scheduler, nodes_dist, batch_size, n_samples, inverse_scaler,
    ...
    def sampling_fn(model, property=None):
        model.eval()
        processed_mols = []
        with torch.no_grad():
            ...
            for r in range(num_sampling_rounds):
                ...
                if property is not None:
                    property1 = torch.stack([property] * n_nodes.shape[0], dim=0)
                    if config.cond_RL:
                        property1= property1 + config.plus
                    context = torch.randn(property1.shape) * config.std + property1
