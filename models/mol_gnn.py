...
class BondFFN(Module):
    def __init__(self,dim, bond_dim, node_dim, inter_dim, use_gate, out_dim=None):
        super().__init__()
        ...
        if self.use_gate:
            self.gate = MLP(dim, out_dim, 32)  # +1 for time
...
class NodeBlock(Module):
    ...
    def __init__(self, node_dim, edge_dim, hidden_dim):
        ...
        self.K = Linear(hidden_dim * 2 + 1024, hidden_dim)
        self.V = Linear(hidden_dim, hidden_dim)
        
        self.node_mlp = Linear(node_dim, hidden_dim)
        self.edge_mlp = Linear(edge_dim, hidden_dim)
        
        self.node_layer_norm = nn.LayerNorm(hidden_dim)
        self.edge_layer_norm = nn.LayerNorm(hidden_dim)
        self.act = nn.ReLU()
        self.node_out_transform = Linear(hidden_dim, node_dim)
        self.edge_out_transform = Linear(hidden_dim, edge_dim)
    ...
    def forward(self, x, edge_index, edge_attr, edge_time):
        ...
        # Compose messages
        Q = self.Q(h_node)
        K = self.K(torch.cat([h_node[col], h_edge, edge_time], dim=-1))
        V = self.V(h_edge * h_node[col])

        # Aggregate messages
        Q_edge = Q[row]
        alpha = (Q_edge * K).sum(dim=-1) / math.sqrt(self.hidden_dim)
        alpha = scatter_softmax(alpha, row)
        
        edge_update = alpha.unsqueeze(-1) * V
        node_update = scatter(edge_update, row, dim=0, dim_size=N)
        
        node_out = self.node_mlp(x) + node_update
        node_out = self.node_layer_norm(node_out)
        node_out = self.node_out_transform(self.act(node_out))
        
        edge_out = self.edge_mlp(edge_attr) + edge_update
        edge_out = self.edge_layer_norm(edge_out)
        edge_out = self.edge_out_transform(self.act(edge_out))
...
class PosUpdate(Module):
    def __init__(self, node_dim, edge_dim, hidden_dim, use_gate):
        ...
        self.edge_lin = BondFFN(1152, edge_dim, edge_dim, node_dim, use_gate, out_dim=1)
...
class EquivariantMixBlock(nn.Module):
    ...
    def __init__(self, node_dim, edge_dim, time_dim, num_extra_heads, num_heads, cond_time, dist_gbf, softmax_inf,
        ...
        if dist_gbf:
            ...
            dist_dim = 1
        self.edge_emb = nn.Linear(edge_dim + dist_dim, edge_dim)
        self.node2edge_lin = nn.Linear(64, edge_dim)

        # message passing layer
        self.nodeblock = NodeBlock(node_dim=256, edge_dim=64, hidden_dim=256)   
        self.posblock = PosUpdate(node_dim=256, edge_dim=64, hidden_dim=256, use_gate=True)    
    ...
    def forward(self, pos, h, edge_attr, edge_index, node_mask, node_time_emb=None, edge_time_emb=None):
        ...
        # apply transformer-based message passing, update node features and edge features (FFN + norm)
        h_node, h_edge = self.nodeblock(h, edge_index, edge_attr, edge_time_emb)
        h_node = h + h_node
        h_edge = edge_attr + h_edge
        h_edge = self.node2edge_lin(h_edge)
        ...
        # apply equivariant coordinate update
        relative_vec, distance1 = self._build_edges_dist(pos, edge_index)
        pos = pos + self.posblock(h_node, h_edge, edge_index, relative_vec, distance1, edge_time_emb)
...
class DGT_concat(nn.Module):
    ...
    def forward(self, t, xh, node_mask, edge_mask, context=None, *args, **kwargs):
        ...
        for i in range(0, self.n_layers):
            h, edge_attr, pos = self._modules['e_block_%d' % i](pos, h, edge_attr, edge_index, node_mask.reshape(-1, 1), node_time_emb, edge_time_emb)
            if self.CoM:
...
class Cond_DGT_concat(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.gate = MLP(time_dim*2, 1, 32)
        self.V = Linear(time_dim, time_dim)
    ...
    def forward(self, t, xh, node_mask, edge_mask, context=None, *args, **kwargs):
        ...
        if self.cond_time:
            ...
            if context is not None:
                gate = self.gate(torch.cat([time_emb, context], dim=-1))
                V = self.V(time_emb * context)
                time_emb = torch.sigmoid(gate) * V
