...
def get_dataset(config, transform=True):
    ...
    # Split dataset
    if 'cond' in config.exp_type and 'RL' not in config.exp_type:
    ...
    train_dataset = dataset.index_select(split_idx['train'].astype(np.int64))
    val_dataset = dataset.index_select(split_idx['valid'].astype(np.int64))
    test_dataset = dataset.index_select(split_idx['test'].astype(np.int64))
